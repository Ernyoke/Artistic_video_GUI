...
from artistic_video.main import stylize
...
class Worker(QThread):
    ...
    def run(self):
        ...
        settings = QSettings()
        content_weight = settings.value(CONTENT_WEIGHT_ID, str(CONTENT_WEIGHT))
        style_weight = settings.value(STYLE_WEIGHT_ID, str(STYLE_WEIGHT))
        tv_weight = settings.value(TV_WEIGHT_ID, str(TV_WEIGHT))
        temporal_weight = settings.value(TEMPORAL_WEIGHT_ID, str(TEMPORAL_WEIGHT))
        learning_rate = settings.value(LEARNING_RATE_ID, str(LEARNING_RATE))
        iterations = settings.value(ITERATIONS_ID, str(ITERATIONS))
        output_location = settings.value(OUTPUT_LOCATION_ID, OUTPUT_LOCATION)
        stylize('imagenet-vgg-verydeep-19.mat', self.content_image, [self.style_image], output_location, iterations, content_weight, style_weight,
                tv_weight, temporal_weight, learning_rate, False)
